...
import { fetchMoviesByGenre, fetchMoviesByRegionLanguage } from "@/services/api";

const categoryChips: string[] = ["Recommended", "TV Series", "Movies", "Genres", "Anime"];
...
const Index = () => {
  const [activeChip, setActiveChip] = useState<string>(categoryChips[0]);

  const { data: topMovies, loading: topLoading, error: topError } = useFetch(() => fetchMoviesByGenre());
  ...
  const isLoading = topLoading || usUkLoading || asianLoading;
  const error = topError || usUkError || asianError;
